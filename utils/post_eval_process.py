...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset="MVTec"):
    ...
    anomaly_types = listdir(str(test_dir))

    for subdir in anomaly_types:
        ...
        test_images = [path.splitext(file)[0]
                    ...
                    #    if path.splitext(file)[1] == ('.png' if object_name == "02" else '.bmp')] # BTAD images are in bmp, VisA images are in png
                       if path.splitext(file)[1] == ('.png' if dataset == "MVTec" else '.JPG')] # VisA images are in JPG, MVTec images are in png
                    #    if path.splitext(file)[1] == ('.jpg' if dataset == "BraTS" else '.JPG')] # BraTS images are in jpg

        # If subdir is not 'good', derive corresponding GT names.
        # if subdir != 'ok':
        if subdir != 'good':
            gt_filenames.extend(
                # [path.join(gt_base_dir, subdir, file + '.bmp' if object_name == "03" else file + '.png') BTAD
                [path.join(gt_base_dir, subdir, file + '_mask.png' if (dataset == "MVTec" or dataset=="BraTS") else file + '.png')
...
def eval_segmentation(object_name, gt_filenames, prediction_filenames, pro_integration_limit=0.3, delete_tiff_files=True):
    ...
    for (gt_name, pred_name) in tqdm(zip(gt_filenames, prediction_filenames),
                                     ...
                                     desc=f"{object_name:<10} Segmentation"):
...
def eval_classification(object_name, gt_filenames, prediction_filenames, aggregation_statistics = "meantop1p"):
    # Read all ground truth and anomaly images.
    ground_truth = []
    predictions = []
    ...
    for (gt_name, pred_name) in tqdm(zip(gt_filenames, prediction_filenames),
                                     total=len(gt_filenames),
                                     desc=f"{object_name:<10} Classification"):
...
def process_object(obj, dataset_base_dir, anomaly_maps_dir, dataset, eval_segm, eval_clf, 
                  pro_integration_limit, delete_tiff_files, aggregation_statistics):
    """
    Process evaluation for a single object
    
    Args:
        obj: The object to evaluate
        dataset_base_dir: Base directory of dataset
        anomaly_maps_dir: Directory containing anomaly maps
        dataset: Dataset name
        eval_segm: Whether to evaluate segmentation
        eval_clf: Whether to evaluate classification
        pro_integration_limit: Integration limit for PRO curve
        delete_tiff_files: Whether to delete TIFF files
        aggregation_statistics: Statistics for aggregation
        
    Returns:
        tuple: (object name, evaluation results dictionary)
    """
    result = dict()
    
    # Parse the filenames for ground truth and predictions
    ...
    if eval_segm:
        ...
        au_pro, auroc_px, f1_px = eval_segmentation(
            object_name=obj,
        ...
        result['seg_AUPRO'] = au_pro
        result['seg_AUROC'] = auroc_px
        result['seg_F1'] = f1_px
    
    if eval_clf:
        # Evaluate classification performance
        auroc_clf, ap_clf, f1_clf = eval_classification(
            object_name=obj,
            gt_filenames=gt_filenames,
            prediction_filenames=prediction_filenames,
            aggregation_statistics=aggregation_statistics)

        result['classification_AUROC'] = auroc_clf
        result['classification_AP'] = ap_clf
        result['classification_F1'] = f1_clf
    return result

def eval_finished_run(dataset, dataset_base_dir, anomaly_maps_dir, output_dir, seed = None, pro_integration_limit = 0.3, eval_clf = True, eval_segm = False, delete_tiff_files = False, aggregation_statistics = "meantop1p"):
    """
    ...
    # Create thread-safe lists to store evaluation metrics
    au_pro_ls = []
    auroc_px_ls = []
    f1_px_ls = []
    auroc_clf_ls = []
    ...
    def process_object_wrapper(obj):
        """Wrapper function to process an object with error handling."""
        try:
    ...
    # Create thread pool with number of workers equal to number of objects
    with ThreadPoolExecutor(max_workers=len(objects)) as executor:
        # Submit all tasks to thread pool
        future_to_idx = {
            executor.submit(process_object_wrapper, obj): i for i, obj in enumerate(objects)
        }
        ...
        # for future in future_to_idx:
            ...
            results[idx] = (obj, result)
        
        # Process results in original order
        for obj, result in results:
            print(f"=== Evaluate {obj} ===")

            evaluation_dict[obj] = result
            # Update performance metric lists in thread-safe manner
            if eval_segm:
                au_pro_ls.append(result['seg_AUPRO'])
                auroc_px_ls.append(result['seg_AUROC']) 
                f1_px_ls.append(result['seg_F1'])
                print(f"Segmentation: AU-PRO: {result['seg_AUPRO']}, AU-ROC: {result['seg_AUROC']}, F1: {result['seg_F1']}")
            if eval_clf:
                auroc_clf_ls.append(result['classification_AUROC'])
                ap_clf_ls.append(result['classification_AP'])
                f1_clf_ls.append(result['classification_F1'])
                print(f"Classification: AU-ROC: {result['classification_AUROC']}, AP: {result['classification_AP']}, F1: {result['classification_F1']}")
    ...
    if eval_segm:
        ...
        print(f"Mean Segmentation: AU-PRO: {evaluation_dict['mean_au_pro']}, AU-ROC: {evaluation_dict['mean_segmentation_au_roc']}, F1: {evaluation_dict['mean_segmentation_f1']}")
    if eval_clf:
        ...
        print(f"Mean Classification: AU-ROC: {evaluation_dict['mean_classification_au_roc']}, AP: {evaluation_dict['mean_classification_ap']}, F1: {evaluation_dict['mean_classification_f1']}")
